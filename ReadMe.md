...
```
C++の関数の引数 or 戻り値は明示的に`&`を指定しなければ参照渡しはされず値渡しをされることになります。この値渡しはint型、float型、char型、ポインタ型などの組み込み型であれば効率的になります。しかし、classやstructなどのユーザー定義型を利用する場合は、多くの場合で値渡しは非効率になり、参照渡しの方が効率的になります。（※多くの場合というだけで必ず効率的になるわけではないので注意が必要です）
