...
- [C++11以降のオブジェクト初期化ベストプラクティス](#c11以降のオブジェクト初期化ベストプラクティス)
  - [はじめに](#はじめに)
  - [未初期化の危険性](#未初期化の危険性)
  - [1. 統一初期化構文（Uniform Initialization）](#1-統一初期化構文uniform-initialization)
  - [2. クラスメンバの初期化](#2-クラスメンバの初期化)
    - [2.1 コンストラクタでの初期化](#21-コンストラクタでの初期化)
    - [2.2 デフォルト初期化とデリゲーティングコンストラクタ](#22-デフォルト初期化とデリゲーティングコンストラクタ)
  - [3. 静的オブジェクトの初期化](#3-静的オブジェクトの初期化)
  - [4. スマートポインタの初期化](#4-スマートポインタの初期化)
  - [まとめ](#まとめ-1)
  - [3.4 ポリモーフィズムのための基底クラスには仮想デストラクタを宣言しよう](#34-ポリモーフィズムのための基底クラスには仮想デストラクタを宣言しよう)
  - [3.5 コンストラクタやデストラクタ内では決して仮想関数を呼び出さないように](#35-コンストラクタやデストラクタ内では決して仮想関数を呼び出さないように)
  - [3.6 値渡しよりconst参照渡しを使おう](#36-値渡しよりconst参照渡しを使おう)
  - [3.7 オブジェクトを戻すべき時に参照を戻そうとしないこと](#37-オブジェクトを戻すべき時に参照を戻そうとしないこと)
  - [3.8 データメンバはprivatc宣 言しよう](#38-データメンバはprivatc宣-言しよう)
  - [3.8 インラインをよく理解しよう](#38-インラインをよく理解しよう)
...
初期化されていないオブジェクトの値を読み出そうとすると、結果は未定義の動作になりま
す。環境によっては、それだけでプログラムが止まってしまうこともあるのです。あるいは、
よくあるのは、たまたまそのオブジェクトの中にあった「でたらめな値」を読み出してしまうことです。それに気がつかないと、プログラムが不思議な動作をするようになり、つらいデバッグ作業をさせられることにもなるのです。
