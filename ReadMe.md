...
Sample_03_10のプログラムのEnemyクラスは、これまでのポリシーに違反しているため、Playerの前方宣言が利用できず、Playerのヘッダーファイルのインクルードが行われています。これをポリシーに従う形に修正して前方宣言を利用できるようにしてください。なお、解答例としてenemy.after.h、enemy.after.cppが用意されています。
...
前節みた、`std::unique_ptr`は排他的なリソース所有権をもつスマートポインタだったため、複数のスマートポインタでメモリを管理することができませんでしたが、`std::shared_ptr`は複数のスマートポインタでメモリを管理することができます。
