...
- [Chapter 3 C++の落とし穴 With Effective C++](#chapter-3-cの落とし穴-with-effective-c)
  ...
  - [3.8 データメンバはprivatc宣言しよう](#38-データメンバはprivatc宣言しよう)
...
### 3.7.1 はじめに
```
値渡しの効率の問題 (20項を参照)が わかると、急に正義のプログラマになって、「邪悪な値渡し」を、 どこに隠れていようと見つけ出し、根絶やしにしてしまおうとする人がいます。しかし、それで致命的な間違いを犯してしまうこともあるのです。それは、「存在しないオブジェクトヘの参照を戻す」という間違いです。
```
...
```
インライン
関数、なんてすばらしいのでしょう!関数のように見え、関数のように動作し、
マクロよりずっとよい(2項を参;1照)、 
